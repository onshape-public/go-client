/*
 * Onshape REST API
 *
 * The Onshape REST API consumed by all clients.
 *
 * API version: 1.113
 * Contact: api-support@onshape.zendesk.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package onshape

import (
	"encoding/json"
)

// BTEdgeTopologyFilter122AllOf struct for BTEdgeTopologyFilter122AllOf
type BTEdgeTopologyFilter122AllOf struct {
	BtType *string `json:"btType,omitempty"`
	IsInternalEdge *bool `json:"isInternalEdge,omitempty"`
}

// NewBTEdgeTopologyFilter122AllOf instantiates a new BTEdgeTopologyFilter122AllOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBTEdgeTopologyFilter122AllOf() *BTEdgeTopologyFilter122AllOf {
	this := BTEdgeTopologyFilter122AllOf{}
	return &this
}

// NewBTEdgeTopologyFilter122AllOfWithDefaults instantiates a new BTEdgeTopologyFilter122AllOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBTEdgeTopologyFilter122AllOfWithDefaults() *BTEdgeTopologyFilter122AllOf {
	this := BTEdgeTopologyFilter122AllOf{}
	return &this
}

// GetBtType returns the BtType field value if set, zero value otherwise.
func (o *BTEdgeTopologyFilter122AllOf) GetBtType() string {
	if o == nil || o.BtType == nil {
		var ret string
		return ret
	}
	return *o.BtType
}

// GetBtTypeOk returns a tuple with the BtType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BTEdgeTopologyFilter122AllOf) GetBtTypeOk() (*string, bool) {
	if o == nil || o.BtType == nil {
		return nil, false
	}
	return o.BtType, true
}

// HasBtType returns a boolean if a field has been set.
func (o *BTEdgeTopologyFilter122AllOf) HasBtType() bool {
	if o != nil && o.BtType != nil {
		return true
	}

	return false
}

// SetBtType gets a reference to the given string and assigns it to the BtType field.
func (o *BTEdgeTopologyFilter122AllOf) SetBtType(v string) {
	o.BtType = &v
}

// GetIsInternalEdge returns the IsInternalEdge field value if set, zero value otherwise.
func (o *BTEdgeTopologyFilter122AllOf) GetIsInternalEdge() bool {
	if o == nil || o.IsInternalEdge == nil {
		var ret bool
		return ret
	}
	return *o.IsInternalEdge
}

// GetIsInternalEdgeOk returns a tuple with the IsInternalEdge field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BTEdgeTopologyFilter122AllOf) GetIsInternalEdgeOk() (*bool, bool) {
	if o == nil || o.IsInternalEdge == nil {
		return nil, false
	}
	return o.IsInternalEdge, true
}

// HasIsInternalEdge returns a boolean if a field has been set.
func (o *BTEdgeTopologyFilter122AllOf) HasIsInternalEdge() bool {
	if o != nil && o.IsInternalEdge != nil {
		return true
	}

	return false
}

// SetIsInternalEdge gets a reference to the given bool and assigns it to the IsInternalEdge field.
func (o *BTEdgeTopologyFilter122AllOf) SetIsInternalEdge(v bool) {
	o.IsInternalEdge = &v
}

func (o BTEdgeTopologyFilter122AllOf) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.BtType != nil {
		toSerialize["btType"] = o.BtType
	}
	if o.IsInternalEdge != nil {
		toSerialize["isInternalEdge"] = o.IsInternalEdge
	}
	return json.Marshal(toSerialize)
}

type NullableBTEdgeTopologyFilter122AllOf struct {
	value *BTEdgeTopologyFilter122AllOf
	isSet bool
}

func (v NullableBTEdgeTopologyFilter122AllOf) Get() *BTEdgeTopologyFilter122AllOf {
	return v.value
}

func (v *NullableBTEdgeTopologyFilter122AllOf) Set(val *BTEdgeTopologyFilter122AllOf) {
	v.value = val
	v.isSet = true
}

func (v NullableBTEdgeTopologyFilter122AllOf) IsSet() bool {
	return v.isSet
}

func (v *NullableBTEdgeTopologyFilter122AllOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBTEdgeTopologyFilter122AllOf(val *BTEdgeTopologyFilter122AllOf) *NullableBTEdgeTopologyFilter122AllOf {
	return &NullableBTEdgeTopologyFilter122AllOf{value: val, isSet: true}
}

func (v NullableBTEdgeTopologyFilter122AllOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBTEdgeTopologyFilter122AllOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
