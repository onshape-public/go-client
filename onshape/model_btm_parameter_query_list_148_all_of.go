/*
 * Onshape REST API
 *
 * The Onshape REST API consumed by all clients.
 *
 * API version: 1.113
 * Contact: api-support@onshape.zendesk.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package onshape

import (
	"encoding/json"
)

// BTMParameterQueryList148AllOf struct for BTMParameterQueryList148AllOf
type BTMParameterQueryList148AllOf struct {
	BtType *string `json:"btType,omitempty"`
	Queries *[]BTMIndividualQueryBase139 `json:"queries,omitempty"`
}

// NewBTMParameterQueryList148AllOf instantiates a new BTMParameterQueryList148AllOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBTMParameterQueryList148AllOf() *BTMParameterQueryList148AllOf {
	this := BTMParameterQueryList148AllOf{}
	return &this
}

// NewBTMParameterQueryList148AllOfWithDefaults instantiates a new BTMParameterQueryList148AllOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBTMParameterQueryList148AllOfWithDefaults() *BTMParameterQueryList148AllOf {
	this := BTMParameterQueryList148AllOf{}
	return &this
}

// GetBtType returns the BtType field value if set, zero value otherwise.
func (o *BTMParameterQueryList148AllOf) GetBtType() string {
	if o == nil || o.BtType == nil {
		var ret string
		return ret
	}
	return *o.BtType
}

// GetBtTypeOk returns a tuple with the BtType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BTMParameterQueryList148AllOf) GetBtTypeOk() (*string, bool) {
	if o == nil || o.BtType == nil {
		return nil, false
	}
	return o.BtType, true
}

// HasBtType returns a boolean if a field has been set.
func (o *BTMParameterQueryList148AllOf) HasBtType() bool {
	if o != nil && o.BtType != nil {
		return true
	}

	return false
}

// SetBtType gets a reference to the given string and assigns it to the BtType field.
func (o *BTMParameterQueryList148AllOf) SetBtType(v string) {
	o.BtType = &v
}

// GetQueries returns the Queries field value if set, zero value otherwise.
func (o *BTMParameterQueryList148AllOf) GetQueries() []BTMIndividualQueryBase139 {
	if o == nil || o.Queries == nil {
		var ret []BTMIndividualQueryBase139
		return ret
	}
	return *o.Queries
}

// GetQueriesOk returns a tuple with the Queries field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BTMParameterQueryList148AllOf) GetQueriesOk() (*[]BTMIndividualQueryBase139, bool) {
	if o == nil || o.Queries == nil {
		return nil, false
	}
	return o.Queries, true
}

// HasQueries returns a boolean if a field has been set.
func (o *BTMParameterQueryList148AllOf) HasQueries() bool {
	if o != nil && o.Queries != nil {
		return true
	}

	return false
}

// SetQueries gets a reference to the given []BTMIndividualQueryBase139 and assigns it to the Queries field.
func (o *BTMParameterQueryList148AllOf) SetQueries(v []BTMIndividualQueryBase139) {
	o.Queries = &v
}

func (o BTMParameterQueryList148AllOf) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.BtType != nil {
		toSerialize["btType"] = o.BtType
	}
	if o.Queries != nil {
		toSerialize["queries"] = o.Queries
	}
	return json.Marshal(toSerialize)
}

type NullableBTMParameterQueryList148AllOf struct {
	value *BTMParameterQueryList148AllOf
	isSet bool
}

func (v NullableBTMParameterQueryList148AllOf) Get() *BTMParameterQueryList148AllOf {
	return v.value
}

func (v *NullableBTMParameterQueryList148AllOf) Set(val *BTMParameterQueryList148AllOf) {
	v.value = val
	v.isSet = true
}

func (v NullableBTMParameterQueryList148AllOf) IsSet() bool {
	return v.isSet
}

func (v *NullableBTMParameterQueryList148AllOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBTMParameterQueryList148AllOf(val *BTMParameterQueryList148AllOf) *NullableBTMParameterQueryList148AllOf {
	return &NullableBTMParameterQueryList148AllOf{value: val, isSet: true}
}

func (v NullableBTMParameterQueryList148AllOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBTMParameterQueryList148AllOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
