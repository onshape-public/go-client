	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, chapterSubelement := range tt.args.subElements {
				btAppElementChangeParams := onshape.NewBTAppElementChangeParams(chapterSubelement.name + strconv.Itoa(i))
			...
			//Path.path element
			pathKey := onshape.NewBTJPathKey3221("NewBTJPathKey-3221")
